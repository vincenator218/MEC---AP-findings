#!/usr/bin/env python3
"""
clear_all_unlocks.py -- force every Unlocks_* ability flag currently set to 1
back down to 0, in one pass, in every ProgressionManagerData* section that
has them. In-place value flips only (like patch_save.py, just for all 58
known names at once instead of one) -- no records are added or removed, so
the file's size and every offset stay exactly the same. Safe, reversible
(the tool prints what it changed and to what, and --out never has to be
your live save unless you choose that).

Why this exists: to test whether the game's own mission-completion state
ever recomputes/re-grants Unlocks_* flags at load time, the cleanest
experiment is to clear ALL of them while leaving whatever missions you've
actually completed exactly as they are, then relaunch. If any flags come
back on their own without you touching them again, that's the game
re-deriving them from mission state. If none do, ability gating is safe
to build a randomizer on top of.

Usage:
    python clear_all_unlocks.py PROF_SAVE --out PROF_SAVE
"""
import argparse
import struct
import sys
