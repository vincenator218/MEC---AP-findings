...
Input : ground_truth.json (2376 flag names from PlayerProgressionData, FINDINGS §12)
Output: locations.json, locations.csv (next to this script)

Every location is one save/live flag. Hash = djb2a(name) -- the same hash used as
the key in PROF_SAVE records (§12) and in the live flag table (§56).

Run:  python build_checks.py ../ground_truth.json
"""
import csv, json, os, re, sys
from collections import Counter

HERE = os.path.dirname(os.path.abspath(__file__))
GT = sys.argv[1] if len(sys.argv) > 1 else os.path.join(HERE, "..", "ground_truth.json")
...
def main():
    names = sorted({f["name"] for f in json.load(open(GT, encoding="utf-8"))["flags"]})
    have = set(names)
    locs = []
    ...
    with open(os.path.join(HERE, "locations.json"), "w", encoding="utf-8") as f:
        json.dump([{c: L[c] for c in cols} for L in locs], f, indent=1, ensure_ascii=False)
    with open(os.path.join(HERE, "locations.csv"), "w", newline="", encoding="utf-8") as f:
